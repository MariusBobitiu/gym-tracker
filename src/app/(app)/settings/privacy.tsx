...
export default function PrivacySettings(): React.ReactElement {
  ...
  const url = resolveLegalUrl("/privacy");
  ...
  useEffect(() => {
    ...
    openModal().catch((error) => {
      ...
      showMessage({
        message: "Unable to open Privacy Policy",
        type: "danger",
        duration: 3000,
      });
    });
  ...
  return (
    ...
    >
      <Stack.Screen options={headerOptions({ title: "Privacy Policy" })} />
      <AppHeader showBackButton title="Privacy Policy" />
      <UIView className="gap-3 px-2 pt-4">
        <P style={{ color: colors.mutedForeground }}>
          The Privacy Policy opens in a secure in-app browser.
        </P>
        <Button
